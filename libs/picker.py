    ...
    try:
        ...
        while True:
            ...
            for i, item in enumerate(lst):
                if i == selected:
                    stdscr.addstr(i+ h//2, w//2-len(item)//2, f"> {item}", curses.color_pair(1))
                else:
                    stdscr.addstr(i+ h//2, w//2-len(item)//2, f"  {item}")
...
class AnsiPicker:
    ...
    def ask(self, timeout=10, default_key = None):
        if not default_key:
            idx = 0
        else:
            idx = list(self.options.keys()).index(default_key)
        out = pick_from_list(list(self.options.keys()), idx, timeout)
